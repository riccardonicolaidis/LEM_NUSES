...
void GeometricalFactor_CZT()
{
    ...
    for(int i = 0 ; i < 3 ; ++i)
    {
        ...
        hIncident[i]   = new TH1D(Form("hIncident_%d", i), Form("Incident energy %d", i), Nbins[i], TMath::Log10(ERange[i][0]), TMath::Log10(ERange[i][1]));
        hIncident[i] -> SetTitle(Form("Incident energy %d: Log10(E[MeV]): Counts", i));
        hAcceptance[i] = new TH1D(Form("hAcceptance_%d", i), Form("Acceptance %d", i), Nbins[i], TMath::Log10(ERange[i][0]), TMath::Log10(ERange[i][1]));        
        hAcceptance[i] -> SetTitle(Form("Accepted events %d: Log10(E[MeV]): Counts", i));
        Edep[i] -> Draw(Form("TMath::Log10(%s)>>hIncident_%d", BranchName[1].Data(), i), "", "");
        Edep[i] -> Draw(Form("TMath::Log10(%s)>>hAcceptance_%d", BranchName[1].Data(), i), ConditionGoodEvents.Data(), "");
        
        NgenCalibrated = NGen * (TMath::Log10(ERange[i][1]) - TMath::Log10(ERange[i][0]))/ (TMath::Log10(ERangeGen[i][1]) - TMath::Log10(ERangeGen[i][0]));;
        dNdEGen = NGen * (hAcceptance[i] -> GetBinWidth(1))/ (TMath::Log10(ERangeGen[i][1]) - TMath::Log10(ERangeGen[i][0]));
    ...
    {
        c1 -> cd();
        hIncident2[i]   = new TH1D(Form("hIncident2_%d", i), Form("Incident energy %d", i), Nbins2[i], TMath::Log10(Xmin), TMath::Log10(Xmax));
        hIncident2[i] -> SetTitle(Form("Incident energy %d: Log10(E[MeV]): Counts", i));
        hAcceptance2[i] = new TH1D(Form("hAcceptance2_%d", i), Form("Acceptance %d", i), Nbins2[i], TMath::Log10(Xmin), TMath::Log10(Xmax));        
        hAcceptance2[i] -> SetTitle(Form("Accepted events %d: Log10(E[MeV]): Counts", i));
        Edep[i] -> Draw(Form("TMath::Log10(%s)>>hIncident2_%d", BranchName[1].Data(), i), "", "");
        Edep[i] -> Draw(Form("TMath::Log10(%s)>>hAcceptance2_%d", BranchName[1].Data(), i), ConditionGoodEvents.Data(), "");
        
        dNdEGen = NGen * (hAcceptance2[i] -> GetBinWidth(1))/ (TMath::Log10(ERangeGen[i][1]) - TMath::Log10(ERangeGen[i][0]));
    ...
    */
    ...
    for(int i = 0 ; i < 3 ; ++i)
    {
        ...
        hFinal2[i] -> GetXaxis() -> SetRangeUser(-1.2,300.);

...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
  ...
  LxDrilledVeto = LActive + 0.8*cm;
  LyDrilledVeto = LActive + 0.8*cm;
  ...
  G4double LExternal = LxPCB - 2*cm;
  G4double LInternal = LxPCB/2 - 1*cm;

    G4double distanceRExternal;
    G4double distanceRInternal;

    for(G4int i = 0; i <N_SENSORS/4;++i)
    {
        for(G4int j= 0; j < N_SENSORS/4;++j)
        {
            CopyNoVector[jCopyNo] = (jCopyNo++);

            xCurrentHole = -LExternal/2 + i*LExternal;
            yCurrentHole = -LExternal/2 + j*LExternal;


            phi = atan2(yCurrentHole, xCurrentHole);
            distanceRExternal = sqrt(xCurrentHole*xCurrentHole + yCurrentHole*yCurrentHole);
            theta = thetaMax;
            ...
            if(i == 0 && j == 0)
            {
                ...
                HolePositionPCB = G4ThreeVector(xCurrentHoleThick, yCurrentHoleThick, zThick);
                HoleRotationPCB = G4RotationMatrix();
                HoleTransformPCB = G4Transform3D(HoleRotationPCB, HolePositionPCB);
    ...
    G4double xTabulated[4] = {LInternal/sqrt(2), -LInternal/sqrt(2), 0, 0};
    G4double yTabulated[4] = {0, 0, LInternal/sqrt(2), -LInternal/sqrt(2)};


    for(G4int i = 0; i <N_SENSORS/2;++i)
    {
        CopyNoVector[jCopyNo] = (jCopyNo++);

        xCurrentHole =  xTabulated[i];
        yCurrentHole =  yTabulated[i];


        phi = atan2(yCurrentHole, xCurrentHole);
        distanceRInternal = sqrt(xCurrentHole*xCurrentHole + yCurrentHole*yCurrentHole);
        theta = thetaMax * distanceRInternal / distanceRExternal;
